// packages/server/src/handlers/budgets/updateBudget.ts
...
export async function updateBudget(req: Request, res: Response) {
  ...
  const parsed = parseAndValidate(updateBudgetSchema, req.body);
  if (!parsed.ok) return send(res, parsed.response);
  const updates = parsed.data as any;

  // 5) Ensure there is at least one field to update
  ...
  try {
    ...
    // 7) Build immutable setPayload for $set
    const setPayload: any = {};

    if (typeof updates.amount !== "undefined")
      setPayload.amount = updates.amount;

    // 8) If categoryId explicitly provided (can be null to remove)
    if (typeof updates.categoryId !== "undefined") {
      ...
      // 9) Category name update: try to resolve user-specific then global
      const cat =
    ...
    // 10) periodStart normalization (if provided)
    ...
    // 11) Always set updatedAt
    setPayload.updatedAt = new Date();

    // 12) If we changed categoryId, ensure no other budget exists for this user+categoryId
    ...
    // 13) Perform atomic update and return updated document
    ...
    // 15) Normalize and return updated document
    ...
    return send(res, jsonResponse(200, { data: resp }));
  } catch (err) {
